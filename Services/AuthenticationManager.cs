...
namespace Services
{
...

    {
        ...
        private JwtSecurityToken GenerateTokenOptions(SigningCredentials ssigninCredentials, List<Claim> claims)
        {
            var jwtSettings = _configuration.GetSection("JwtSettings");
            var tokenOptions = new JwtSecurityToken(
                issuer: jwtSettings["validIssuer"],
                audience: jwtSettings["validAuidience"],
                claims: claims,
                expires: DateTime.Now.AddMinutes(Convert.ToDouble(jwtSettings["expires"])),
                signingCredentials: ssigninCredentials);
        ...
        private ClaimsPrincipal GetPrincipalFromExpiredToken(string token)
        {
            ...
            var tokenValidationParameters= new TokenValidationParameters
            {
                ...
                ValidAudience = jwtSettings["validAuidience"],
        ...
        public async Task<TokenDto> RefreshToken(TokenDto tokenDto)
        {
           var principal=GetPrincipalFromExpiredToken(tokenDto.AccessToken);
            ...
            if (user is null ||
                user.RefreshToken != tokenDto.RefreshToken || user.RefreshTokenExpiryTime <= DateTime.Now)
